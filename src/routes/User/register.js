...
export default class Register extends Component {
  ...
  getPasswordStatus = () => {
    ...
    const value = form.getFieldValue('FPsw');
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields({ force: true }, (err, values) => {
      values['FPsw']=Utility.MD5(values['FPsw']);
      values['confirm']=Utility.MD5(values['confirm']);
      ...
      if (!err) {
        this.props.dispatch({
          ...
          callback:function(result){
             if(result['result']==1){
               message.success(result['alerMes']);
             }else if(result['result']==-1){
                message.error(result['alerMes']);
             }
          },
        });
      }
  ...
  checkConfirm = (rule, value, callback) => {
    const { form } = this.props;
    if (value && value !== form.getFieldValue('FPsw')) {
      callback('两次输入的密码不匹配!');
    } else {
      callback();
    }
